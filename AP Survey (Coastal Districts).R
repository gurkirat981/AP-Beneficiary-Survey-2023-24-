...
#Main Files ####
AP_Beneficiary_Survey <- read_excel("/Users/gurkirat/Library/Mobile Documents/com~apple~CloudDocs/Desktop/Flags Drp/Beneficiary_Survey_Nov10.xlsx")
AP_Beneficiary_Roster <- read_excel("/Users/gurkirat/Library/Mobile Documents/com~apple~CloudDocs/Desktop/Flags Drp/Beneficiary_Survey_Nov10.xlsx", sheet = 2)
...
matchednames <- read_excel("/Users/gurkirat/Library/Mobile Documents/com~apple~CloudDocs/Desktop/Flags Drp/enumerators details (coastal districts).xlsx", sheet = 2)
...
AB <- merge(AB, matchednames, by.x = c(8), by.y = c(1))

AB <- AB %>% 
  mutate (District = ifelse(District == "NTR", "NTR District",
                            ifelse(District == "East  Godavari", "East Godavari", District)))
...
AB <- AB %>% 
filter(start < '2023-08-15 00:00:00')
...
matches <- merge (imperfect_matches, used_names, by.x = c(14), by.y = c(1))
...
a <- as.data.frame(table(sec_names_phase2$NEW_DISTRICT[sec_names_phase2$No_Responses > 14])) %>% 
rename("District" = Var1) %>% 
rename("Secretariats Completed" = Freq)

districts <- as.data.frame(table(all_secretariats$NEW_DISTRICT[all_secretariats$Completed == 0])) %>% 
rename("District" = Var1) %>% 
rename("Secretariats Sampled" = Freq)

table_secretariats <- merge (a, districts, by.x = c(1), by.y = c(1)) %>% 
filter (District == "BAPATLA" | District == "EAST GODAVARI" | District == "ELURU" | District == "GUNTUR" | District == "KAKINADA" |
        District == "KONASEEMA"| District == "KRISHNA" | District == "NTR" | District == "PALNADU" | District == "PRAKASAM" |
...
AS <- AB %>% 
  mutate (S = 1) %>% 
  group_by(Enumerator_Code, `Enumerator Name`, District) %>% 
  summarise (n= sum(S)) %>% 
  ungroup ()
...
write_xlsx(BS, "Phase II Submissions.xlsx")
...
am <- APCoast_Roster_Merged %>% 
  group_by(District, `_submission__id`) %>% 
  summarise (member_households = max(SNo)) %>% 
  ungroup()
...
DRP$total <- rowSums(DRP[,c(2:66)])
DRP[, "max"] <- apply(DRP[, c(2:66)], 1, max)

ARP <- as.data.frame.matrix(table(AB$Enumerator_Code, AB$date))
ARP[ARP > 0] <- 1
ARP$total <- rowSums(ARP[,c(1:65)])
#ARP[ARP == 1] <- "P"
#ARP[ARP == 0] <- "A"
ARP <- rownames_to_column(ARP)

attendance_sheet <- merge(matchednames, ARP, by.x = c(1), by.y = c(1)) %>% 
rename ("Phase II Total working days" = total)

progress_sheet <- merge(matchednames, DRP, by.x = c(1), by.y = c(1)) %>% 
  rename ("Phase II Total submissions" = total)

write_xlsx(attendance_sheet, "Phase II Attendance Sheet.xlsx")
write_xlsx(progress_sheet, "Phase II total submimssions.xlsx")

#Week V: Enumerator Productivity by Districts
max_surveys <- DRP[, c(1, 67:68)] %>% 
  rename("Enumerator_Code" = rowname) 

max_days <- ARP[, c(1, 67:67)] %>% 
  rename("Enumerator_Code" = rowname) %>% 
  rename("total days" = total)

max_days_surveys <- merge (max_surveys, max_days, by.x = c(1), by.y = c(1))

max_days_surveys <- merge(matchednames, max_days_surveys, by.x = c(1), by.y = c(1))

district_productivity <- max_days_surveys %>% 
group_by(District) %>% 
summarise(No_Enum = length(`Enumerator Code`), No_days = sum(`total days`), No_Surveys = sum(total)) %>% 
ungroup()

write_xlsx(district_productivity, "Phase II Enumerator Productivity by Districts.xlsx")


3#Enumerators by districts
enum_districts <- AB %>% 
...
b <- as.data.frame(table(AB$Enumerator_Code[AB$submission_time < 10])) %>% 
rename("Enumerator Code" = Var1) %>% 
rename ("Number of Surveys" = Freq)

surveys_under10 <- merge (AS, b, by.x = c(1), by.y = c(1))

write_xlsx(surveys_under10, "Week VIII Surveys under 10 minutes.xlsx")


par(family = "Times New Roman", font = 2, font.lab = 2, font.axis = 2)
hist_bh <- hist(AB$submission_time[AB$submission_time < 45 & AB$submission_time > 0], xlim = c(0,50), ylim = c(0,110), 
xlab = "Submission Time (in minutes)", main = "Week VIII & IX: Distribution of surveys by submission time", 
ylab = "Number of Surveys (Frequency)", col = "skyblue2", border = "white", labels = TRUE)

#High-performing and Under-Performing Enumerators
enum_check <- merge(submission_time, max_surveys, by.x = c(1), by.y = c(1)) %>% 
...
male_responses <- merge(matchednames, male_responses, by.x = c(1), by.y = c(1)) %>% 
  rename("Gender = Male" = Freq)

write_xlsx(male_responses, "gender reported male.xlsx")

#Running checks on individual enumerators
pradeep <- AB %>% 
filter (`Enumerator Name`== "G Pradeep")

pradeep_roster <- APCoast_Roster_Merged %>% 
  filter (`Enumerator Name`== "G Pradeep")

raju <- AB %>% 
  filter (`Enumerator Name`== "P. Raju")

raju_roster <- APCoast_Roster_Merged %>% 
  filter (`Enumerator Name`== "P. Raju")

govindaiah <- AB %>% 
filter (`Enumerator Name` == "Govindaiah")

wilson <- AB %>% 
filter(`Enumerator Name`== "YENIBERA WILSON")

vinod <- AB %>% 
filter (Enumerator_Code == 54468)
